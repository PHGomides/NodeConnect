...
* **`exceptions/`**: Nossos erros customizados para facilitar o debug.
  * *Exemplo:* `DecryptionFailedException.java`, `HandshakeNotCompletedException.java`.
...
* *Exemplo:* `SecurityService.java` (Interface), `AesGcmProvider.java`, `KeyManager.java`, `EcdhKeyExchange.java`, `HandshakeService.java`.
...
## 📂 `src/test/java/com/nodeconnect/`

Espelha a estrutura de `src/main/java/`. Cada módulo do código de produção tem sua pasta correspondente aqui, contendo os testes unitários daquele módulo. O nome de cada classe de teste segue o padrão `{ClasseTestada}Test.java`.

### 📦 `crypto/`
Testes unitários da camada de segurança. Validam o ciclo completo de cifragem/decifragem, integridade do MAC, e o handshake ECDH.
* *Exemplo:* `AesGcmProviderTest.java`, `EcdhKeyExchangeTest.java`, `HandshakeServiceTest.java`.